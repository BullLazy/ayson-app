...
class AysonApp(App):
    ...
    def open_history_popup(self, *_):
        ...
        scroll = ScrollView(do_scroll_x=False, do_scroll_y=True)
        items_box = BoxLayout(orientation="vertical", spacing=dp(10), size_hint_y=None)
        ...
        if not self.history:
            ...
            items_box.add_widget(empty)
        else:
            for item in self.history:
                items_box.add_widget(self._history_row(item))
    ...
    def _history_row(self, item):
        row = RoundedBox(orientation="vertical", padding=dp(10), spacing=dp(8), size_hint_y=None, height=dp(154), bg_color=CARD)
        ...
        show_btn = GhostButton(text="Goster", height=dp(42))
        show_btn.font_size = dp(12)
        ...
        show_btn.bind(on_press=lambda *_args, it=item: self.show_history_item(it))
    ...
    def show_history_item(self, item):
        try:
            ...
            pass
        self.last_result = item.get("result", "") if item.get("status") != "captcha" else ""
        self.output.text = item.get("result", "")
        self.set_open_button(item.get("result", ""))
        self.set_status("Gecmis acildi", SUCCESS if item.get("status") != "captcha" else WARNING)
